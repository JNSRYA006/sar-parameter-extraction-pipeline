...
case 7 %JONSWAP (gamma,Hs,w0):
    Hs	  = Par(1:height(Par),1:length(Par)/3);
    w0    = Par(1:height(Par),length(Par)/3+1:2*length(Par)/3);
    gamma = Par(1:height(Par),2*length(Par)/3+1:end);
    g=9.81;
    alpha=0.2.*Hs.^2.*w0.^4./g.^2;
    if gamma(1)<1 || gamma(1) > 7  %DNV recommeded values when gamma is unknown.
		if gamma(1) ~= 0
    ...
    for k=1:1:length(W)
            ...
            %nonLin = exp(3.3-0.03.*(gamma-1)./sigma)))
    ...
    TitleStr='JONSWAP Spectrum';
    %L1Str = ['gamma =',num2str(gamma(1)),' Hs =',num2str(Hs),' [m],','  w0=',num2str(w0),' [rad/s]'];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
