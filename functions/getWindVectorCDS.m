function [U_10,V_10,long,lat,t,citation] = getWindVectorCDS(metadata)
...
area_meta = formatLatLonValues(metadata,attribute_names);
...
[time_meta, day_meta, month_meta, year_meta] = formatDateValues(metadata,attribute_names);
...
function area = formatLatLonValues(metadata,attribute_names)

% Get all lat values
lat_near1 = metadata(strcmp({metadata.Name},attribute_names(2))).Value;
lat_far1 = metadata(strcmp({metadata.Name},attribute_names(4))).Value;
lat_near2 = metadata(strcmp({metadata.Name},attribute_names(6))).Value;
lat_far2 = metadata(strcmp({metadata.Name},attribute_names(8))).Value;
lat = [lat_near1,lat_near2,lat_far1,lat_far2];

% Get all long values
lon_near1 = metadata(strcmp({metadata.Name},attribute_names(3))).Value;
lon_far1 = metadata(strcmp({metadata.Name},attribute_names(5))).Value;
lon_near2 = metadata(strcmp({metadata.Name},attribute_names(7))).Value;
lon_far2 = metadata(strcmp({metadata.Name},attribute_names(9))).Value;
